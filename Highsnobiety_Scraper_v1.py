...
def scrape_articles(driver, output1, page, month, year):
    ...
    for _ in range(100):  
        try:
            ...
            try:
                wait(driver, 1).until(EC.presence_of_element_located((By.CSS_SELECTOR, "button[class*='nextButton']")))
                next_button = True
            except:
                pass
            ...
            if next_button:
                try:
                    posts = wait(driver, 4).until(EC.presence_of_all_elements_located((By.CSS_SELECTOR, "article[class*='teaser']")))  
                except:
                    break

                try:
                    wait(driver, 1).until(EC.presence_of_element_located((By.TAG_NAME, "time")))
                    art_time = 'ago'
                except:
                    art_time = 'month'
                ...
                for post in posts:
                    try:
                        if art_time == 'month':
                            date = wait(post, 1).until(EC.presence_of_element_located((By.CSS_SELECTOR, "span[class*='date']"))).get_attribute('textContent') 
                            ...
                            link = wait(post, 1).until(EC.presence_of_element_located((By.CSS_SELECTOR, "a[class*='teaser']"))).get_attribute('href')
                            if link not in links:
                                links.append(link)            
                        else:
                            date = wait(post, 1).until(EC.presence_of_element_located((By.TAG_NAME, "time"))).get_attribute('textContent') 
                            if 'months' in date or 'year' in date or 'hour' in date: continue
                            link = wait(post, 1).until(EC.presence_of_element_located((By.CSS_SELECTOR, "a[data-cy*='teaser']"))).get_attribute('href')
                            if link not in links:
                                links.append(link)
                    except:
                        pass
            ...
            try:
                button = wait(driver, 1).until(EC.presence_of_element_located((By.CSS_SELECTOR, "button[class*='loadMoreButton']")))
            ...
            try:
                button = wait(driver, 1).until(EC.presence_of_element_located((By.CSS_SELECTOR, "button[class*='nextButton']")))
                driver.execute_script("arguments[0].click();", button)
                time.sleep(3)
            except:
                pass
            ...
            try:
                ...
                try:
                    art_time = wait(driver, 3).until(EC.presence_of_all_elements_located((By.CSS_SELECTOR, "span[class*='date']")))[-1].get_attribute('textContent').strip().lower()
                except:
                    break

            try:
                if 'months' in art_time or 'year' in art_time: 
                    break
            ...
            if height1 == height2 and not next_button: 
                break
    ...
    for i, link in enumerate(links):
        try:
            ...
            art_id = ''
            try:
                text = wait(driver, 4).until(EC.presence_of_element_located((By.TAG_NAME, "article"))).get_attribute('data-id').strip()
            ...
            author, date = '', ''             
            try:
                author = wait(driver, 4).until(EC.presence_of_element_located((By.CSS_SELECTOR, "span[class*='author']"))).get_attribute('textContent').split('\n')[-1].replace("Words By", "").strip()
                div = wait(driver, 4).until(EC.presence_of_element_located((By.CSS_SELECTOR, "div[class*='articleInfo']")))
                date = wait(div, 4).until(EC.presence_of_element_located((By.TAG_NAME, "time"))).get_attribute('datetime').strip()
            except Exception as err:
                pass
            ...
            try:
                ...
                art_day = int(date.split('-')[2][:2])
                if art_month < prev_month:
                    print(f'skipping the remaining articles because they are from {art_month}/{art_year}')
                    break
            ...
            title = ''             
            try:
                title = wait(driver, 4).until(EC.presence_of_element_located((By.CSS_SELECTOR, "h2[class*='articleTitle']"))).get_attribute('textContent').strip()
            except:
                continue               
            ...
            des = ''             
            try:
                div = wait(driver, 4).until(EC.presence_of_element_located((By.CSS_SELECTOR, "div[class*='articleContent']")))
                elems = wait(div, 4).until(EC.presence_of_all_elements_located((By.TAG_NAME, "p")))
                for elem in elems:
                    try:
                        text = elem.get_attribute('textContent').strip()
                        if 'Want to keep browsing?' in text: continue
                        des += text + '\n'
                    except:
                        pass
                des = des.strip('\n')
            ...
            cat = ''             
            try:
                cat = wait(driver, 4).until(EC.presence_of_element_located((By.CSS_SELECTOR, "a[class*='category']"))).get_attribute('textContent').strip().title()
            except:
                pass 
            ...
            tags = ''
            try:
                ul = wait(driver, 4).until(EC.presence_of_element_located((By.CSS_SELECTOR, "ul[class*='tag']")))
                elems = wait(ul, 4).until(EC.presence_of_all_elements_located((By.CSS_SELECTOR, "li[class*='tag']")))
                for elem in elems:
                    try:
                        text = elem.get_attribute('textContent').strip(' ').strip(',').title()
                        tags += text + ', '
                    except:
                        pass
                tags = tags.strip(', ')
            ...
            # other columns
            row['domain'] = 'Highsnobiety'
            row['hype'] = 0   
            row['articletags'] = tags
            row['articleheader'] = ''

            imgs = ''
            try:
                art = wait(driver, 4).until(EC.presence_of_element_located((By.TAG_NAME, "article")))
                elems = wait(art, 4).until(EC.presence_of_all_elements_located((By.CSS_SELECTOR, "img[class*='image']")))
                for elem in elems:
                    try:
                        ...
                        url = url[url.rindex('www.highsnobiety.com'):]
                        url = url.split('/30x20/')[-1]
                        imgs += url.strip() + ', '
                    except:
                        pass
                imgs = imgs.strip(', ')
...
def get_inputs():
    ...
    path = os.getcwd()
    if '\\' in path:
        path += '\\Highsnobiety_settings.xlsx'
    else:
        path += '/Highsnobiety_settings.xlsx'

    if not os.path.isfile(path):
        print('Error: Missing the settings file "Highsnobiety_settings.xlsx"')
...
def initialize_output():
    ...
    file1 = f'Highsnobiety_{stamp}.xlsx'
